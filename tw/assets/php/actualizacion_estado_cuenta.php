	...
	while ($data = fgetcsv ($ubicacion, 1000, ";")) {
		...
		if ( abs(str_replaceChars($separar[4]))>0 OR str_replaceChars($separar[5])>0 ) {
			...
			if ( count($separar_texto)>5 ) {
				...
				WHERE a.cuenta="'.$cuenta.'" AND estatus=0';
		...
		if($tipo>0) {
			...
			if($rmovimiento > 0){
				...
				$sql="INSERT INTO `saldo_bancos` (`id`, `fecha`, `idcuenta`, `fecha_banco`, `hora_banco`, `movimiento`, `cuenta`, `rastreo`, `tipo`, `descripcion`,`importe`,`idcuenta_empresa`,`fpago`) VALUES ('', '".date('Y-m-d')."', '".$idcunetaxcliente."', '".$separar[0]."', '".$separar[1]."', '".$separar[2]."', '".$cuenta."', '".$no_rastreo."', '".$tipo."', '".$texto."', '".$importe."', '".$idcuenta_empresa."', '".$fpagox."' )";
