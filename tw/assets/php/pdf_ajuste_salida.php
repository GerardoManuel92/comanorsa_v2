...
{ 
    ...
    function Header(){
        ...
        $this->SetY(15);$this->SetX(130); $this->MultiCell(70,1,utf8_decode("Ajuste de salida"),0,'C',0);///////TITULO
    ...
    public function GeneraContenido(  
        ...
        date_default_timezone_set('America/Mexico_City');
            ...
            $selectPart = "SELECT a.cantidad,a.costo,a.tot_total,b.nparte,b.descripcion, a.iva, a.tot_iva, a.cantidad,b.nparte AS clave,c.marca AS marcax, b.img, a.idparte, d.abr AS unidad, e.observaciones as motivo
                            FROM partes_ajuste_salida a, alta_productos b, alta_marca c, sat_catalogo_unidades d, alta_ajuste_salida e
                            WHERE
                            a.idparte=b.id 
                            AND e.id = a.idajuste
                            AND b.idmarca = c.id
        ...
        while(  $row=$db->sql_fetchrow($buscar3)   ){
           ...
           $last_yimg = $valory+25;
            ...
            $suma_subtotal = $suma_subtotal+$row['costo'];
            ...
            if ($valor_incremento > 270) {
                ...
                $this->SetX(169);$this->MultiCell(29,0,wims_currency($row['costo']), 0, 'R', 0);
...
$query = 'SELECT a.fecha, a.observaciones, b.nombre AS usuario, b.correo AS correo, b.telefono AS telefono
            FROM alta_ajuste_salida a, alta_usuarios b
            WHERE
            a.idusuario=b.id
...
$sql = "UPDATE `alta_ajuste_salida` SET `documento` = '".$folioX."' WHERE `id`=".$folio;
$db->sql_query($sql);
