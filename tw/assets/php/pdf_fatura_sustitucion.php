...
{ 
    ...
    public function GeneraContenido(  
        ...
        $selectPart = "SELECT e.id,b.nparte AS clave, CONCAT_WS(' ',b.clave,e.descripcion,d.marca) AS descrip, c.abr AS unidad, e.costo, e.iva,e.descuento,(e.cantidad*e.costo) AS subtotal,e.cantidad,e.idparte,
            ROUND( ( ((e.cantidad*e.costo)- ((e.cantidad*e.costo)*(e.descuento/100)))*(e.iva/100)),2 ) AS tiva, 
            ROUND( ( (e.cantidad*e.costo)*(e.descuento/100)),2 ) AS tdescuento,
            d.marca AS marcax
                FROM partes_factura_sustitucion e, alta_productos b, sat_catalogo_unidades c, alta_marca d
                WHERE
                e.idparte=b.id
                AND b.idunidad=c.id
                AND b.idmarca = d.id
        ...
        if ( $valory > 182 ) {
            ...
            $this->SetY($last_y+23);
            $this->SetX(61);
            $this->MultiCell(50, 0, $UUID, 0, 'R', 0); ///////TOTAL
...
$query = 'SELECT x.ftimbrado,x.htimbrado,x.moneda,x.tcambio,x.cadena,x.dias,x.idcotizacion,
b.nombre AS cliente, b.comercial, c.nombre AS vendedor,h.calle,h.exterior,h.colonia,h.interior,h.municipio,h.estado,h.cp, 
...
FROM alta_factura_sustitucion x, alta_clientes b, alta_usuarios c, sat_catalogo_fpago d, sat_metodo_pago e, sat_catalogo_cfdi f, folio_factura g, direccion_clientes h
WHERE
x.idcliente=b.id
AND x.idusuario=c.id
AND x.idfpago=d.id
AND x.idmpago=e.id
AND x.idcfdi=f.id
AND x.idfolio=g.id
...
$sqlupt = "UPDATE alta_factura_sustitucion SET cadena='".$cadena."', uuid='".$UUID."' WHERE id=".$idfact;
$db->sql_query($sqlupt);
