...
$objPHPExcel = $objReader->load("plantilla/plantilla_complemento_pago.xlsx");

$objPHPExcel->setActiveSheetIndex(0); //seleccionar hora

$objPHPExcel->getActiveSheet()->setTitle("COMPLEMENTOS_PAGO"); //establecer titulo de hoja
...
} else if (strlen($texto) > 0 && $status > 0) { 
...
if (strlen($texto) > 0 && $status == 0) {
  ...
  AND CONCAT_WS(' ',e.nombre,e.comercial,f.folio,a.uuid,a.total) LIKE '%" . $texto . "%' 
...
while ($row2 = $db->sql_fetchrow($buscar2)) {
  ...
  /**** CONSULTA PARA OBTENER FACTURA CORRESPONDIENTE AL IPPDX */

  $sql3 = "SELECT a.idfactura, CONCAT_WS('',c.serie,c.folio) AS facturax 
  ...
  $objPHPExcel->getActiveSheet()->SetCellValue("E$numFila", implode(', ', $facturaxArray));
...
$objWriter->save('reporte/reporte_complementos_pago.xlsx');
