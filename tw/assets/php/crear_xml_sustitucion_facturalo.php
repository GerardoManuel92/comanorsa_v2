...
FROM `alta_factura_sustitucion` a, sat_catalogo_fpago b, sat_catalogo_cfdi c, sat_metodo_pago d
WHERE 
a.idfpago=b.id
AND a.idcfdi=c.id
and a.idmpago=d.id
...
$sql_ivax = "SELECT DISTINCT(a.iva) AS diferentes_ivas 
			FROM partes_factura_sustitucion a
			WHERE
...
$sql4 = "SELECT d.costo,d.cantidad,d.descripcion,b.sat,c.clave AS clave_unidad,c.abr AS unidad, b.nparte, d.iva, d.descuento, d.riva, d.risr
FROM partes_factura_sustitucion d, alta_productos b, sat_catalogo_unidades c
WHERE
d.idparte=b.id
AND b.idunidad=c.id
AND d.estatus = 0
...
<cfdi:Comprobante xsi:schemaLocation="http://www.sat.gob.mx/cfd/4 http://www.sat.gob.mx/sitio_internet/cfd/4/cfdv40.xsd" Version="4.0" Serie="'.$serie.'" Folio="'.$SerieFolio.'" Fecha="'.date("Y-m-d").'T'.$hora_actual.'" SubTotal="'.$subTotal.'" Descuento="'.$totdescuento.'" Moneda="'.$moneda.'" TipoCambio="'.$tipo_cambio.'" Total="'.$total_real.'" TipoDeComprobante="I" Exportacion="01" Certificado="'.$certificado.'" NoCertificado="'.$nocertificado.'" MetodoPago="'.$metodo_pago.'" FormaPago="'.$forma_pago.'" LugarExpedicion="'.$cp_emisor.'" xmlns:cfdi="http://www.sat.gob.mx/cfd/4" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
...
$ubicacion = 'xml/sarchivo_xml'.$idfactura.'.xml';
	...
	switch($opc)
	{
		...
		case 31: 
	        	...
	        	if ( $respuesta['codigo'] == 200 ) {
					...
					$sql_update = "UPDATE alta_factura_sustitucion SET ftimbrado = '".date('Y-m-d')."', htimbrado='".$hora_actual."',idfolio = (SELECT MAX(x.id) FROM folio_factura x), estatus='1' WHERE id=".$idfactura;
					$db->sql_query($sql_update);

					/////////////******************* CAMBIAMOS EL ESTATUS DE LA COTIZACION COMO FACTURA TIMBRADA (no existe cotizacion)

					/////***************LIMPIAMOS LA TABLA DE TEMPORALES PARTES SUSTITUCION 

					$sql_truncate = "DELETE FROM temporal_partes_nueva_factura WHERE idusuario=".$idusuario;
					$db->sql_query($sql_truncate);
