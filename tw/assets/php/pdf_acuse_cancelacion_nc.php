...
{ 
    ...
    public function GeneraContenido(  
        ...
        $xml = simplexml_load_file('acuse_cancelacion_nc/acuse'.$folio_fact.'.xml');
        ...
        $this->SetY(110.2);
        $this->SetX(95);
        $this->MultiCell(50, 0,$estatus." - ".$txt_est, 0, 'C', 0); ///////importe con letra
...
//$folio_fact = "A7055";
$idfactura= trim($_POST["idfactura"]);//IDNC

/////////////***************** DATOS GENERALES 

$sql_info = "SELECT comercial,rfc,razon_social,direccion FROM `datos_generales` WHERE estatus = 0";
...
$sql_cliente = "SELECT b.nombre AS cliente, b.rfc AS rfc_cliente, a.uuid, a.total,CONCAT_WS('',c.serie,c.folio) AS folio_fact
FROM alta_nota_credito a, alta_factura d,alta_clientes b, folio_nota_credito c
WHERE d.idcliente=b.id
AND a.idfolio=c.id
...
$nombrearchivo = "facturas_canceladas_nc/cancelacion_".$folio_fact.".pdf";
