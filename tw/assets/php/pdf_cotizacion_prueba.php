...
{ 
    ...
    function Header(){
        ...
        $this->SetY(37);$this->SetX(50); $this->MultiCell(15,2,"Cliente:",0,'L',0);///////TITULO
        ...
        $this->SetY(37);$this->SetX(61); $this->MultiCell(130,2,strtoupper( utf8_decode($cliente) ),0,'L',0);///////TITULO
        ...
        $this->SetDrawColor(0,0,0);
        $this->SetY(15);$this->SetX(130); $this->MultiCell(70,1,utf8_decode("COTIZACIÓN"),0,'C',0);///////TITULO
    ...
    public function GeneraContenido(  
        ...
        date_default_timezone_set('America/Mexico_City');
            ...
            $selectPart = "SELECT a.id,b.nparte AS clave, CONCAT_WS('',a.descripcion) AS descrip, c.abr AS unidad, a.costo, a.iva,a.tot_subtotal AS subtotal,a.cantidad,a.idparte,a.tot_iva AS tiva,a.tot_descuento AS tdescuento, d.marca AS marcax, b.img, a.tcambio, a.tot_total AS total
                    FROM partes_cotizacion a, alta_productos b, sat_catalogo_unidades c, alta_marca d
                    WHERE a.idparte=b.id
                    AND b.idunidad=c.id
                    AND b.idmarca = d.id
                    AND a.idcotizacion = ".$idcot."
                    AND a.estatus = 0";
        ...
        while(  $row=$db->sql_fetchrow($buscar3)   ){
           ...
           $last_yimg = $valory+25;
            ...
            $suma_iva = $suma_iva+round($row['tiva'],2);
            $suma_descuento = $suma_descuento+round($row['tdescuento'],2);
        ...
        if ( $valory > 240 ) {
            ...
            $this->MultiCell(190, 3, utf8_decode("**Vigencia: ".$dias." días hábiles del mes en curso o hasta agotar existencias, imágenes con fines ilustrativos el producto real puede variar"), 0, 'L', 0); ///////importe con letra
        ...
        $sql_upt="UPDATE alta_cotizacion SET subtotal='".$suma_subtotal."', descuento='".$suma_descuento."', iva='".$suma_iva."', total='".$total."' WHERE id=".$idcot;
...
$idcot =2290;//trim($_POST["idcot"]);//40;//trim($_POST["idcot"]);

$query = 'SELECT a.fcotizacion, a.observaciones, b.nombre AS cliente, b.comercial, c.nombre AS vendedor, CONCAT_WS(" / ",b.telefono,b.celular) AS telefonos,b.correo,d.descripcion AS fpago,b.rfc,a.moneda,a.tcambio
FROM alta_cotizacion a, alta_clientes b, alta_usuarios c, sat_catalogo_fpago d
WHERE
a.idcliente=b.id
...
$fecha = obtenerFechaEnLetra($row2["fcotizacion"]);
...
$dir = 'SELECT calle,exterior,interior,colonia,municipio,estado,cp FROM `direccion_clientes` WHERE idcliente=(SELECT x.idcliente FROM alta_cotizacion x WHERE x.id='.$idcot.') LIMIT 0,1';
$buscar_dir = $db->sql_query($dir);
$row_dir = $db->sql_fetchrow($buscar_dir);

$calle = $row_dir["calle"];
$colonia = $row_dir["colonia"];
$exterior = $row_dir["exterior"];
$interior = $row_dir["interior"];
$municipio = $row_dir["municipio"];
$estado = $row_dir["estado"];
$cp = $row_dir["cp"];
...
$nombrearchivo = "cotizaciones/cot".$idcot.".pdf";
//$pdf->Output($nombrearchivo, 'F');
$pdf->Output();

echo json_encode($idcot);
