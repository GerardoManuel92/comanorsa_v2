...
$idcliente =21;//trim($_POST["idcli"]);
$idfactura =215;//trim($_POST["idfact"]);
$idcotizacion=685;//trim($_POST["idcot"]);
$name_factura="ALEX prueba";//trim($_POST["name_factura"]);


$tipox =1;//trim($_POST["tipo"]);

if ( $tipox == 1 ) {

	//// se va a refacturar este cfdi

	$mpagox =2;//trim($_POST["mpagox"]);
	$fpagox =21;//trim($_POST["fpagox"]);
	$cfdix =23;//trim($_POST["cfdix"]);
	$odcx ="";//trim($_POST["odc"]);
...
while($row4=$db->sql_fetchrow($buscar4)){
	...
	if ( $vdescuento > 0 ) {
		...
		$ti_descuento = redondedoFloat( ($vdescuento/100),2 );
	...
	$ti_iva = redondedoFloat( ($row4["iva"]/100),2 );
	...
	if ( $idcfdi == 23 ) {
		...
		if ( $vdescuento > 0 ) {
			...
			$ti_descuentox =redondedoFloat( ($vdescuento/100),2 );
		...
		$partes_xml = $partes_xml.'<cfdi:Concepto Cantidad="'.$ti_cantidad.'" ClaveProdServ="'.$ti_clavesat.'" ClaveUnidad="'.$ti_clv_unidad.'" Descripcion="'.$ti_descripcion.'" Iva="'.$ti_costo_iva.'" Costox="'.$ti_costo.'" Importe="'.redondedoFloat($ti_importe_obj_impuesto,2).'"  NoIdentificacion="'.$ti_clave.'" Unidad="'.$ti_unidad.'" ValorUnitario="'.redondedoFloat($ti_costo_obj_impuesto,2).'" Descuento="'.$descuento.'" ObjetoImp="'.$obj_impuesto.'">';
		...
		$obj_impuesto = "02";
			...
			$partes_xml = $partes_xml.'<cfdi:Concepto Cantidad="'.$ti_cantidad.'" ClaveProdServ="'.$ti_clavesat.'" ClaveUnidad="'.$ti_clv_unidad.'" Descripcion="'.$ti_descripcion.'" Importe="'.$ti_importe.'" NoIdentificacion="'.$ti_clave.'" Unidad="'.$ti_unidad.'" ValorUnitario="'.$ti_costo.'" Descuento="'.$descuento.'" ObjetoImp="'.$obj_impuesto.'">
		      <cfdi:Impuestos>
		        <cfdi:Traslados>
		          <cfdi:Traslado Base="'.$ti_importe_iva.'" Importe="'.$total_iva.'" Impuesto="002" TasaOCuota="'.$ti_iva.'" TipoFactor="Tasa" />
		        </cfdi:Traslados>';
...
if ( $idcfdi == 23 ) {
	...
	if ( $total_retenciones > 0 ) {
		...
		for ($i=0; $i<count($array_iva); $i++) { 
			...
			$cadena_xml 	.='<cfdi:Traslado Base="'.$sumar_importe.'" Importe="'.redondedoFloat($sumar_iva,2).'" Impuesto="002" TasaOCuota="'.redondedoFloat( ($array_iva[$i]/100),2 ).'" TipoFactor="Tasa" />';
