...
$objPHPExcel = $objReader->load("plantilla/plantilla_cliente_individual.xlsx");
...
$cliente = $_POST["idclientex"];



if ($cliente > 0) {

  $sql = "SELECT a.id,a.estatus,CONCAT_WS('/',a.id,a.estatus) AS acciones, a.nombre,a.comercial, a.rfc,CONCAT_WS(' ', a.calle, a.exterior, a.interior, a.colonia, a.municipio, a.estado, a.cp) AS direccion,
  ...
  while ($row = $db->sql_fetchrow($buscar)) {
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue("C4", $idcliex . '.- ' . $nombre_clientex);
...
$sql2 = "SELECT a.nombre AS contacto, a.puesto, a.telefono, a.correo 

          FROM contactos_erp a 

          WHERE a.iddepartamento = 2 

          AND a.iduser= '" . $cliente . "'";
...
//////////********************* MOSTRAR CUENTAS BANCARIAS DEL CLIENTE 

$sql3 = "SELECT a.id,a.cuenta,b.rfc,b.comercial 

          FROM cuentas_bancariasxcliente a, alta_bancos b 
          
          WHERE a.idbanco=b.id 

          AND a.idcliente= '" . $cliente . "'
...
$objWriter->save('reporte/CLIENTES/' . $cliente . '.- '. $nombre_clientex .'.xlsx');
