...
/***************************************************************************
	...
	*Observaciones 						   								   *
	...
	- el regimen fiscal no fue añadido revisarlo con july				   *
								                                           *
								                                           *
	544 euromundo - 156 - ventas al PUBLICO
	587 sin timbrar monto 51 mil - ventas al publico 							                                           *
								                                           *
								                                           *
								                                           *
								                                           *
...
$idcliente = 18;//trim($_POST["idcli"]);
$idfactura = 156;//trim($_POST["idfact"]);
$idcotizacion= 544;//trim($_POST["idcot"]);
...
while ( $row_iva=$db->sql_fetchrow($buscar4) ) {
 ...
 ////////////*************** colocar las partidas en los nodos 
	...
	if ( $idcfdi == 23 ) {
		...
		$obj_impuesto = "02";
			...
			$partes_xml = $partes_xml.'<cfdi:Concepto Cantidad="'.$ti_cantidad.'" ClaveProdServ="'.$ti_clavesat.'" ClaveUnidad="'.$ti_clv_unidad.'" Descripcion="'.$ti_descripcion.'" Importe="'.$ti_importe.'" NoIdentificacion="'.$ti_clave.'" Unidad="'.$ti_unidad.'" ValorUnitario="'.$ti_costo.'" Descuento="'.redondedoFloat($descuento,2).'" ObjetoImp="'.$obj_impuesto.'">
		      <cfdi:Impuestos>
		        <cfdi:Traslados>
		          <cfdi:Traslado Base="'.$ti_importe_iva.'" Importe="'.$total_iva.'" Impuesto="002" TasaOCuota="'.round($ti_iva,6).'" TipoFactor="Tasa" />
		        </cfdi:Traslados>';
...
if ( $idcfdi == 23 ) {
	...
	//$subTotalx= $subTotalvpublico-redondedoFloat($totdescuento,2);

	//echo json_encode("Subtotal: ".$subTotal."//Iva: ".$ivax."//descuento: ".redondedoFloat($totdescuento,2));
...
if ( $idcfdi == 23 ) {
	...
	if ( $total_retenciones > 0 ) {
		...
		for ($i=0; $i<count($array_iva); $i++) { 
			...
			$cadena_xml 	.='<cfdi:Traslado Base="'.$sumar_importe.'" Importe="'.redondedoFloat($sumar_iva,2).'" Impuesto="002" TasaOCuota="'.round( ($array_iva[$i]/100),6 ).'" TipoFactor="Tasa" />';
...
$ubicacion = 'xml/archivo_xml_euro.xml';
$archivo = fopen($ubicacion, "w+");
fwrite($archivo, $cadena_xml);

echo json_encode("imagenes orden aleatorio<br>");

echo json_encode($array_suma_iva);
