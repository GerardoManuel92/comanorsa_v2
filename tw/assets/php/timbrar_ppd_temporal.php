                        ...
                        /////*************** LO TIMBRAMOS CON LA NUEVA FUNCION PARA OBTENER EL QR DEL XML 
                            ...
                            if ( file_exists($nombre_fichero) ) {
                                ...
                                if ( $imagenEnBase64 != "" ) {
                                    ...
                                    //////////*************************** CAMBIAMOS EL ESTATUS DEL PAGO EN SALDO DE BANCOS POR EL IDMOVIMIENTO


                                    $importe_movimiento=trim($_POST['importe_movimiento']);

                                    $sql_pago='SELECT IFNULL(SUM(total),0) AS tot_pago
                                                FROM alta_pago_cxc WHERE idmovimiento="'.$idmov.'"
                                                UNION ALL
                                                SELECT IFNULL(SUM(a.pago),0) AS tot_pago   
                                                FROM alta_pagos_ppd a
                                                WHERE a.idppd=(SELECT x.id FROM alta_datos_ppd x WHERE x.idmovimiento_bancario="'.$idmov.'" limit 0,1)';


                                    $pbuscar=$db->sql_query($sql_pago);

                                    $ptotal_pago=0;

                                    while($prow=$db->sql_fetchrow($pbuscar)){
                                        
                                        $ptotal_pago=$ptotal_pago+$prow["tot_pago"];

                                    }

                                    $prestante=$importe_movimiento-$ptotal_pago;

                                    if( $prestante==0 OR $prestante<0 ){

                                        /////////// CAMBIAR ES ESTATUS DEL SALFDO DEL BANCO A APLICADO

                                        $act_saldo="UPDATE saldo_bancos SET estatus=1 WHERE id=".$idmov;
                                        $db->sql_query($act_saldo);

                                    }

                                    /*$sql_update_movimiento = "UPDATE saldo_bancos SET estatus='1' WHERE id=".$idmov;

                                    $db->sql_query($sql_update_movimiento);*/
