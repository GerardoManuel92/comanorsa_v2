<?php
        
        namespace TIMBRADORXPRESS\API;
        require __DIR__ . "/facturalo/class.conexion.php";
        use TIMBRADORXPRESS\API\Conexion;

        header('Content-Type: application/json');
        include("config.php"); 
        include("includes/mysqli.php");
        include("includes/db.php");
        set_time_limit(600000);
        date_default_timezone_set('America/Mexico_City');

        function changeString($string)
        {
         
            $string = trim($string);
         
            $string = str_replace(
                array('á', 'à', 'ä', 'â', 'ª', 'Á', 'À', 'Â', 'Ä'),
                array('a', 'a', 'a', 'a', 'a', 'A', 'A', 'A', 'A'),
                $string
            );
         
            $string = str_replace(
                array('é', 'è', 'ë', 'ê', 'É', 'È', 'Ê', 'Ë'),
                array('e', 'e', 'e', 'e', 'E', 'E', 'E', 'E'),
                $string
            );
         
            $string = str_replace(
                array('í', 'ì', 'ï', 'î', 'Í', 'Ì', 'Ï', 'Î'),
                array('i', 'i', 'i', 'i', 'I', 'I', 'I', 'I'),
                $string
            );
         
            $string = str_replace(
                array('ó', 'ò', 'ö', 'ô', 'Ó', 'Ò', 'Ö', 'Ô'),
                array('o', 'o', 'o', 'o', 'O', 'O', 'O', 'O'),
                $string
            );
         
            $string = str_replace(
                array('ú', 'ù', 'ü', 'û', 'Ú', 'Ù', 'Û', 'Ü'),
                array('u', 'u', 'u', 'u', 'U', 'U', 'U', 'U'),
                $string
            );
         
            $string = str_replace(
                array('ñ', 'Ñ', 'ç', 'Ç'),
                array('n', 'N', 'c', 'C',),
                $string
            );
         
            //Esta parte se encarga de eliminar cualquier caracter extraño
            $string = str_replace(
                array('º', '~','!','&','´',';','"','°'),
                array('','','','&amp;','','','&quot;',' grados'),
                $string
            );
         
         
            return $string;
        }

        $url = 'https://app.facturaloplus.com/ws/servicio.do?wsdl';
        $objConexion = new Conexion($url);

        # CREDENCIAL
        //$apikey='4056008b83334c1d84f6776c6144beba';

        $tdocumento=trim($_POST["documento"]);
        $id_documento=trim($_POST["idfact"]);
        $motivo=trim($_POST["motivox"]);
        $obsx=trim($_POST["obsx"]);
        $folio_factura=trim($_POST["foliox"]);
        $idfacturax=trim($_POST["idfacturax"]);
        $texto_factura=trim($_POST["textofactura"]);

        /*$tdocumento=1;//trim($_POST["documento"]);
        $id_documento=695;//trim($_POST["idfact"]);
        $motivo=2;//trim($_POST["motivox"]);
        $obsx="OBSERVACION";//trim($_POST["obsx"]);
        $folio_factura="BDL7653";//trim($_POST["foliox"]);
        $idfacturax='';//200;//trim($_POST["idfacturax"]);
        $texto_factura="BDL7467 | $ 11,680.30 | CF54C522-15A4-4EA3-8CFC-4E7373A11D49 ";;//trim($_POST["textofactura"]);*/

        switch ($tdocumento) {

            case 1:
                
                ///// FACTURA
                $sql1="SELECT a.uuid, b.rfc, a.total,( SELECT clave FROM motivo_cancelacion WHERE id='".$motivo."' LIMIT 0,1 ) AS mcancelacion
                FROM alta_factura a, alta_clientes b
                WHERE a.idcliente=b.id
                AND a.id=".$id_documento;
               

            break;

            case 4:

                ///// factura sustitucion

             $sql1="SELECT a.uuid, b.rfc, a.total,( SELECT clave FROM motivo_cancelacion WHERE id='".$motivo."' LIMIT 0,1 ) AS mcancelacion
                FROM alta_factura_sustitucion a, alta_clientes b
                WHERE a.idcliente=b.id
                AND a.id=".$id_documento;
                
            break;

        }

                $buscar1=$db->sql_query($sql1);
                $row1=$db->sql_fetchrow($buscar1);

                $uuid=trim($row1["uuid"]);
                $rfcReceptor=trim($row1["rfc"]);
                $total=trim($row1["total"]);
                $mcancelacion=trim($row1["mcancelacion"]);


        /////********** MOTIVO DE CANCELACION

        if( $mcancelacion == 1 ){

            /////SEPARAMOS EL UUID 

            $separar=explode("|", $texto_factura);
            $uuid_sustitucion=trim($separar[2]); 

            $folioSustitucion=$uuid_sustitucion;

        }else{

            $folioSustitucion="";

        }


        //////*********** DATOS DEL EMISOR
        $sql="SELECT rfc, apikey FROM `datos_generales` WHERE estatus = 0";
        $buscar = $db->sql_query($sql);
        $row = $db->sql_fetchrow($buscar);

        $rfcEmisor = trim($row["rfc"]);
        $apikey = trim($row["apikey"]);
        /*$cerCSD = "";

    $keyCSD="";

    $passCSD="Ca261316";*/



$cerCSD = "";

$keyCSD="";

$passCSD="C64526452";


    /*$datos= array('APIKEY' => $apikey,'PASSCSD' => $passCSD,'UUID' => $uuid, 'EMISOR' => $rfcEmisor, 'RECEPTOR' => $rfcReceptor, 'TOTAL' => $total, 'OBS' => $obsx , 'SUSTITUCION' => $folioSustitucion);

    echo json_encode($datos);*/


    $respuesta = false;

    $resultado = $objConexion->operacion_cancelar($apikey, $keyCSD, $cerCSD, $passCSD, $uuid, $rfcEmisor, $rfcReceptor, $total, $mcancelacion, $folioSustitucion, $folio_factura);

    //echo $resultado;

    $separarx=explode("%", $resultado);//0:exitosa,1:declinada||0:acusexml,1:mensaje de error||0:no existe dato,1:codigo de error;

    //if ( $resultado != null ) {

    //echo $separarx[0];


    if ( $separarx[0] == 0 ){


        switch ($tdocumento) {

            case 1:
            
                /////******** ACTUALIZAR ESTATUS DE LA FACTURA 
                $updatex = "UPDATE `alta_factura` SET `estatus` = '2', `obs_cancelacion` = '".$obsx."', `rcancelacion` = '".$idfacturax."', `uuid_sustitucion` = '".$folioSustitucion."'  WHERE `id`=".$id_documento;
                $db->sql_query($updatex);
                     
            break;

            case 4:
                
                /////******** ACTUALIZAR ESTATUS DE LA FACTURA 
                $updatex = "UPDATE `alta_factura_sustitucion` SET `estatus` = '2', `obs_cancelacion` = '".$obsx."', `rcancelacion` = '".$idfacturax."', `uuid_sustitucion` = '".$folioSustitucion."'  WHERE `id`=".$id_documento;
                $db->sql_query($updatex);


            break;


        }


        
       

        ## GUARDAR ACUSE EN DIRECTORIO ACTUAL ##
        //file_put_contents('acuse_cancelacion/acuse'.$folio_factura.'.xml', $resultado);

        file_put_contents('acuse_cancelacion/acuse'.$folio_factura.'.xml', $separarx[1]);
        $respuesta = 0;
        
    }else{


        $respuesta=$separarx[2].'-'.$separarx[1];

    }

    echo json_encode($respuesta);

    //echo json_encode($uuid);

?>