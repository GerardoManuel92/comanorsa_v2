...
$idfactura_anterior=trim($_POST["idcot"]);
...
/////////// DATOS DE LA FACTURA ANTERIOR A SUSTITUIR

$sql_anterior = "SELECT a.idcotizacion,a.uuid FROM alta_factura a WHERE id=".$idfactura_anterior;
$consulta = $db->sql_query($sql_anterior);
$rconsulta = $db->sql_fetchrow($consulta);

$idcotizacion= trim($rconsulta["idcotizacion"]);
$uuid_anterior= trim($rconsulta["uuid"]);
	...
	if ( $idcfdi == 23 ) {
		...
		$obj_impuesto = "02";
			...
			$partes_xml = $partes_xml.'<cfdi:Concepto Cantidad="'.$ti_cantidad.'" ClaveProdServ="'.$ti_clavesat.'" ClaveUnidad="'.$ti_clv_unidad.'" Descripcion="'.$ti_descripcion.'" Importe="'.$ti_importe.'" NoIdentificacion="'.$ti_clave.'" Unidad="'.$ti_unidad.'" ValorUnitario="'.$ti_costo.'" Descuento="'.$descuento.'" ObjetoImp="'.$obj_impuesto.'">
		      <cfdi:Impuestos>
		        <cfdi:Traslados>
		          <cfdi:Traslado Base="'.$ti_importe_iva.'" Importe="'.$total_iva.'" Impuesto="002" TasaOCuota="'.redondedoFloat($ti_iva,2).'" TipoFactor="Tasa" />
		        </cfdi:Traslados>';
...
<cfdi:CfdiRelacionados TipoRelacion="04">
        <cfdi:CfdiRelacionado UUID="'.$uuid_anterior.'" />
