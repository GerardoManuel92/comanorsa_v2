...
var cantidada_solicitada=0;
var cantidada_asignada=0;
var costo_proveedor=0;
var precio_cliente=0;
var descuento_proveedor=0;//(%)
    ...
    if (original !== e.target.textContent) {
      ...
      if (colx == 0 || colx == 1) {
        ...
        if(colx == 5){
          ...
          //console.log("Requerido"+cantidada_requerida+" Asignar"+cantidada_asignada);

          separar=cantidada_requerida.split("/");

          requeridax=separar[1];
          solicitada_odcx=separar[0];

          total_requerida=parseFloat(requeridax)+parseFloat(solicitada_odcx);

          asignadox=e.target.textContent;

          verificar=0;

          if(parseFloat(asignadox)>parseFloat(total_requerida)){
        ...
        if(colx==6){
          ...
          costox=e.target.textContent.replace(/[^\d.]/g, "");
          preciox=precio_cliente.replace(/[^\d.]/g, "");

          descuento_aplicado=descuento_proveedor;
          

          if(parseFloat(descuento_aplicado)>0){

            descuentox=descuento_aplicado/100;
            costo_real=costox-(costox*descuentox);
          ...
          if( parseFloat(costo_real)>parseFloat(preciox) ){

            console.log("costox: "+costo_real+"||"+"preciox: "+preciox+" descuentox"+descuento_proveedor);
        ...
        if(colx==8){
            ...
            descx=e.target.textContent;

            if(parseFloat(descx)>100){
        ...
        if(verificar==0){
          ...
          $.ajax({
            type: "POST",
            dataType: "json",
            url: base_urlx + "AsgProveedorv2/updateCelda/",
            data: {
              texto: e.target.textContent.replace(/[^\d.]/g, ""),
              columna: colx,
              idprq: editId,//idparte 
            },
            cache: false,
            success: function (result) {
              ...
              if (result) {
                ...
                /*if (colx == 8) {
                  ...
                  temp[8] = (round(e.target.textContent));
                  temp[11] = formatNumber(round(result));
                  ...
                  temp[10] = (e.target.textContent);
                  temp[11] = formatNumber(round(result));
                  $('#my-table').dataTable().fnUpdate(temp, idFila, undefined, false);
                }
                else if (colx == 5) {
                  temp[5] = (e.target.textContent);                
                  $('#my-table').dataTable().fnUpdate(temp, idFila, undefined, false);
                }
                //table.ajax.reload();
                sumaTotal();*/

                temp[5]=result.solicitar;
                temp[6]=formatNumber(round(result.costo_proveedor));
                temp[8]=result.descuento;
                //temp[10]=result.
...
function showDias(idprox) {
  ...
  $.ajax({
    ...
    url: base_urlx + "AsgProveedorv2/showDias/",
...
function showxproveedor() {
  ...
  if ($("#proveedor").val() > 1) {
    ...
    table = $('#my-table').DataTable({
      ...
      "serverSide": true,
      "search": false,
      "ajax": base_urlx + "AsgProveedorv2/loadPartidaspro?idpro=" + $("#proveedor").val(),
      "lengthMenu": [[20, 30, 50, -1], [20, 30, 50, "All"]],
      "scrollY": 300,
      "scrollX": true
    });
    ...
    table = $('#my-table').DataTable({
      ...
      "serverSide": true,
      "search": false,
      "ajax": base_urlx + "AsgProveedorv2/loadPartidas?cot=0",
      "lengthMenu": [[20, 30, 50, -1], [20, 30, 50, "All"]],
      "scrollY": 300,
      "scrollX": true
    });
...
function showProveedor() {
  ...
  $.ajax({
    type: "POST",
    dataType: "json",
    url: base_urlx + "AsgProveedorv2/showProveedor/",
    cache: false,
    success: function (result) {
      if (result != null) {
...
function showDocumentos() {
  ...
  $.ajax({
    type: "POST",
    dataType: "json",
    url: base_urlx + "AsgProveedorv2/showDocumentos/",
    cache: false,
    success: function (result) {
      if (result != null) {
        ...
        $.each(result, function (i, item) {
          ...
          creaOption = creaOption + '<option value="' + data1 + '"> RQ -' + data2 + '</option>';
        });
...
function asignarPartidas() {
  ...
  if (verificar == 0) {
    ...
    $.ajax({
      type: "POST",
      dataType: "json",
      url: base_urlx + "AsgProveedorv2/asignarParteOC/",
      data: {
        idpoc: menu,
        iduser: iduserx,
        idpro: $("#proveedor_asig").val()
...
function enviarCorreo(idcotx) {
  ...
  $.ajax({
    ...
    url: base_urlx + "AsgProveedorv2/enviarCorreo",
...
function DatosAsignar(idpartex) {
  ...
  if (idpartex > 0) {
    ...
    $.ajax({
      ...
      url: base_urlx + "AsgProveedorv2/DatosAsignar/",
...
function AsignarProveedor() {
  ...
  if (revision == 0) {
    ...
    $.ajax({
      ...
      url: base_urlx + "AsgProveedorv2/Asignarproveedor/",
...
function calcularBalance(idpartex, solicitado) {
  ...
  $.ajax({
    ...
    url: base_urlx + "AsgProveedorv2/calcularBalance/",
...
function calcularPrecio(asignado_prov, costo_prov, descuento_prov) {
  
  costo_prov =costo_prov.replace(/[^\d.]/g, "");
  ...
  if(descuento_prov>0){
    ...
    costo_desc=parseFloat(costo_prov)-(costo_prov*(descuento_prov/100));

  }else{

    costo_desc=costo_prov;

  }

  subtotal_prov=asignado_prov*costo_desc;

  return formatNumber(round(subtotal_prov));
}
  ...
  if ( x ) {
    ...
    if ( verificar == 0 ) {
      ...
      $.ajax({
            type: "POST",
            dataType: "json",
            url: base_urlx+"AsgProveedorv2/finalizarOc/",
            data:{
...
$("#documento").change(function () {
  ...
  table = $('#my-table').DataTable({
    "language": {
      "lengthMenu": "Mostrando _MENU_ articulos por pagina",
      "zeroRecords": "No hay partidas que mostrar",
      //"info": "Pagina _PAGE_ de _PAGES_ | Total _TOTAL_ <br>",
    ...
    "fnRowCallback": function (nRow, aData, iDisplayIndex) {
      ...
      if (true) // your logic here
      {
        ...
        $('td:eq(0)', nRow).addClass('alignCenter').addClass('fontText2').addClass('bgcolor1');
        $('td:eq(1)', nRow).addClass('alignCenter').addClass('fontText4');
        $('td:eq(4)', nRow).addClass('alignCenter').addClass('color_blue').addClass('bold');
        $('td:eq(5)', nRow).addClass('alignCenter').addClass('color_red').addClass('bold');
        $('td:eq(3)', nRow).addClass('fontText2');
        $('td:eq(6)', nRow).addClass('alignCenter').addClass('color_green').addClass('bold');
        $('td:eq(7)', nRow).addClass('alignCenter').addClass('color_green').addClass('bold');
        $('td:eq(8)', nRow).addClass('alignCenter').addClass('bold');
        $('td:eq(9)', nRow).addClass('alignRight');
        $('td:eq(10)', nRow).addClass('alignRight');
        $('td:eq(11)', nRow).addClass('alignRight').addClass('color_blue').addClass('bold');
    ...
    columnDefs: [
      ...
      },      
      {
        ...
        data: null,
        render: function (data, type, row, meta) {
          valor = calcularPrecio(data[5], data[6], data[8]);
          return `` + valor + ``
        }
      },  
      /*
    ...
    "serverSide": true,
    "search": false,
    "ajax": base_urlx + "AsgProveedorv2/loadPartidas?rq="+$("#documento").val(),
  ...
  $('#my-table tbody').on('click', 'tr', function () {
    ...
    var data = table.row(this).data();
    var expresionRegular = /(\d+\.\d+)\/(\d+\.\d+)/;
    var cantidades = data[4].match(expresionRegular);

    cantidadMax = cantidades[2]-cantidades[1];
  ...
  $('#my-table tbody').on('focusin', 'tr', function () {
    ...
    cantidada_requerida=data[4];
    cantidada_asignada=data[5];
    costo_proveedor=data[6];

    precio_cliente=data[7];
    descuento_proveedor=data[8];
    
    //console.log(precio_cliente);
    //console.log($("#proveedor_asig").val());
    //idFila = table.row( this ).index();
    //idColumn = table.cell( this ).index().column;
  });
