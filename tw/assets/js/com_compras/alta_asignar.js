    ...
    if (original !== e.target.textContent) {
      ...
      if (colx == 0 || colx == 1) {
        ...
        if(colx == 5){
          ...
          if((parseFloat(e.target.textContent) > parseFloat(cantidadMax)) || (parseFloat(e.target.textContent)<=0)){
            row.invalidate();
            return;
          }
        ...
        if (colx == 5) {
          if (parseFloat(e.target.textContent) > parseFloat(solicitarx)) {
            ...
            validar = 0
            console.log(parseFloat(e.target.textContent) > parseFloat(solicitarx));
          }
        } */
        ...
        $.ajax({
          type: "POST",
          dataType: "json",
          url: base_urlx + "AsgProveedor/updateCelda/",
          data: {
...
function showDias(idprox) {
  ...
  $.ajax({
    ...
    url: base_urlx + "AsgProveedor/showDias/",
...
function showxproveedor() {
  ...
  if ($("#proveedor").val() > 1) {
    ...
    table = $('#my-table').DataTable({
      ...
      "serverSide": true,
      "search": false,
      "ajax": base_urlx + "AsgProveedor/loadPartidaspro?idpro=" + $("#proveedor").val(),
      "lengthMenu": [[20, 30, 50, -1], [20, 30, 50, "All"]],
      "scrollY": 300,
      "scrollX": true
    });
    ...
    table = $('#my-table').DataTable({
      ...
      "serverSide": true,
      "search": false,
      "ajax": base_urlx + "AsgProveedor/loadPartidas?cot=0",
      "lengthMenu": [[20, 30, 50, -1], [20, 30, 50, "All"]],
      "scrollY": 300,
      "scrollX": true
    });
...
function showProveedor() {
  ...
  $.ajax({
    type: "POST",
    dataType: "json",
    url: base_urlx + "AsgProveedor/showProveedor/",
    cache: false,
    success: function (result) {
      if (result != null) {
...
function showDocumentos() {
  ...
  $.ajax({
    type: "POST",
    dataType: "json",
    url: base_urlx + "AsgProveedor/showDocumentos/",
    cache: false,
    success: function (result) {
      if (result != null) {
...
function asignarPartidas() {
  ...
  if (verificar == 0) {
    ...
    $.ajax({
      type: "POST",
      dataType: "json",
      url: base_urlx + "AsgProveedor/asignarParteOC/",
      data: {
        idpoc: menu,
        iduser: iduserx,
        idpro: $("#proveedor_asig").val()
...
function enviarCorreo(idcotx) {
  ...
  $.ajax({
    ...
    url: base_urlx + "AsgProveedor/enviarCorreo",
...
function DatosAsignar(idpartex) {
  ...
  if (idpartex > 0) {
    ...
    $.ajax({
      ...
      url: base_urlx + "AsgProveedor/DatosAsignar/",
...
function AsignarProveedor() {
  ...
  if (revision == 0) {
    ...
    $.ajax({
      ...
      url: base_urlx + "AsgProveedor/Asignarproveedor/",
...
function calcularBalance(idpartex, solicitado) {
  ...
  $.ajax({
    ...
    url: base_urlx + "AsgProveedor/calcularBalance/",
...
function calcularPrecio(costo, iva, descuento) {
  ...
  costo = parseFloat(costo.replace(/[^\d.]/g, ""));
  ...
  const precioFinal = precioConIVA * (1 - descuento / 100);
  const precioFinalRedondeado = Math.round(precioFinal * 100) / 100;

  return formatNumber(precioFinalRedondeado);
}
  ...
  if ( x ) {
    ...
    if ( verificar == 0 ) {
      ...
      $.ajax({
            type: "POST",
            dataType: "json",
            url: base_urlx+"AsgProveedor/finalizarOc/",
            data:{
  ...
  table = $('#my-table').DataTable({
    ...
    columnDefs: [
      ...
      },      
      {
        ...
        data: null,
        render: function (data, type, row, meta) {
          valor = calcularPrecio(data[10], data[9], data[8]);
          return `` + valor + ``
        }
      },
      /*
    ...
    "serverSide": true,
    "search": false,
    "ajax": base_urlx + "AsgProveedor/loadPartidas?rq="+$("#documento").val(),
