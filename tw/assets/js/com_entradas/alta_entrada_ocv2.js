...
$(document).ready(function() {
  ...
  table = $('#my-table').DataTable({
        ...
        "ajax": base_urlx+"EntradaOcv2/loadPartidas?idoc="+$("#idoc").val(),
...
function entradaParcial(){
  ...
  if ( x ) {
    ...
    $('#my-table').DataTable().rows().data().each(function(el, index){
      ...
      clastEntry = el[4]; // Acumulación de entradas
      centrada = el[5]; //Entrada ajustada
      idAsignarOC = el[10]; // Id parte asignada
      cvalor = codc+"/"+centrada+"/"+idAsignarOC+"/"+clastEntry;
    ...
    $("#btn_parcial").html('Almacenando...');
        ...
        $.ajax({
          type: "POST",
          dataType: "json",
          url: base_urlx+"EntradaOcv2/entradaParcial/",
          data:{
            info:info_parcial,
            iduser:iduserx,
            obs:$("#obs").val(),
...
function entradaAll(){
  ...
  if ( x ) {
    ...
    $.ajax({
          type: "POST",
          dataType: "json",
          url: base_urlx+"EntradaOcv2/entradaAll/",
          data:{
            info:info_parcial,
            iduser:iduserx,
            obs:$("#obs").val(),
...
function finalizarEntrada(id) {
  ...
  if ( x ) {
    ...
    $.ajax({
          type: "POST",
          dataType: "json",
          url: base_urlx+"EntradaOcv2/finalizarEntrada/",
          data:{
            info:killRow,
            iduser:iduserx,
            obs:$("#obs").val(),
