...
/*         if (colx == 7) {
        ...
        $.ajax({
          type: "POST",
          dataType: "json",
          url: base_urlx + "AltaRq/updateCelda/",
          data: {

            cantidadRq: e.target.textContent,
            columna: colx,
            idRq: editId,
            idproveedor: $("#proveedor_asig").val(),
            idusuario:iduserx
...
function calcularRQ(pedido, odc, almacen, asignado_amacen) {

  pedido = parseInt(pedido);
  odc = parseInt(odc);
  almacen = parseInt(almacen);
  asignado_amacen = parseInt(asignado_amacen);
...
function showxproveedor() {
  ...
  if ($("#proveedor").val() > 1) {
    ...
    table = $('#my-table').DataTable({
      ...
      "serverSide": true,
      "search": false,
      "ajax": base_urlx + "AltaRq/loadPartidaspro?idpro=" + $("#proveedor").val(),
      "lengthMenu": [[20, 30, 50, -1], [20, 30, 50, "All"]],
      "scrollY": 300,
      "scrollX": true
    });
    ...
    table = $('#my-table').DataTable({
      ...
      "serverSide": true,
      "search": false,
      "ajax": base_urlx + "AltaRq/loadPartidas?cot=0",
      "lengthMenu": [[20, 30, 50, -1], [20, 30, 50, "All"]],
      "scrollY": 300,
      "scrollX": true
    });
...
function showProveedor() {
  ...
  $.ajax({
    ...
    url: base_urlx + "AltaRq/showProveedor/",
...
function showDocumentos() {
  ...
  $.ajax({
    ...
    url: base_urlx + "AltaRq/showDocumentos/",
...
function asignarPartidas() {
  ...
  if (verificar == 0) {
    ...
    $.ajax({
      type: "POST",
      dataType: "json",
      url: base_urlx + "AltaRq/asignarPartidas/",
      data: {
        idpoc: menu,
        iduser: iduserx,
        idpro: $("#proveedor_asig").val()
...
function enviarCorreo(idcotx) {
  ...
  $.ajax({
    ...
    url: base_urlx + "AltaRq/enviarCorreo",
...
function DatosAsignar(idpartex) {
  ...
  if (idpartex > 0) {
    ...
    $.ajax({
      ...
      url: base_urlx + "AltaRq/DatosAsignar/",
...
function AsignarProveedor() {
  ...
  if (revision == 0) {
    ...
    $.ajax({
      ...
      url: base_urlx + "AltaRq/Asignarproveedor/",
...
function calcularBalance(idpartex, solicitado) {
  ...
  $.ajax({
    ...
    url: base_urlx + "AltaRq/calcularBalance/",
  ...
  if ( x ) {
    ...
    if ( verificar == 0 ) {
      ...
      $.ajax({
            type: "POST",
            dataType: "json",
            url: base_urlx+"AltaRq/finalizarOc/",
            data:{
...
$("#documento").change(function () {
  ...
  table = $('#my-table').DataTable({
    ...
    columnDefs: [
      {
        ...
        render: function (data, type, row, meta) {
          ...
          valor = calcularRQ(data[4], data[5], data[6], 0); 
          return `` + valor + ``
        }
      },
      {
        ...
        createdCell: function (cell, cellData, rowData, row, col) {
          var valor = calcularRQ(rowData[4], rowData[5], rowData[6], 0); 
          if (valor > 0) {          
    ...
    "serverSide": true,
    "search": false,
    "ajax": base_urlx + "AltaRq/loadPartidas?cot="+$("#documento").val(),
