  ...
  if ( table !="" ) {
   ...
   table = $('#my-table').DataTable({
      ...
      "language": {
        ...
        "serverSide": true,
        "search" : false,
        "ajax": base_urlx+"Rcpp/loadPartidas?buscar="+$("#buscador").val() + "&estatusx=" + $("#bestatus").val(),
    ...
    $('#my-table tbody').on('click', 'tr', function () {
        ...
        $("#datos_cfdi").html('<i class="fa fa-file" style="color:darkblue;" ></i><a href="'+base_urlx+'/tw/php/facturas_ppd/'+fol_cpp+'.pdf" target="_blank" style="color:darkblue;"> '+fol_cpp+'</a> - '+clientex);
    } );
...
function showContacto(idcliente){
  ...
  $.ajax({
            type: "POST",
            dataType: "json",
            url: base_urlx+"Rcpp/showContacto/",
  ...
  if ( verificar == 0 ) {
      ...
      if ( x ) {  
        ...
        $.ajax({
                type: "POST",
                dataType: "json",
                url: base_urlx+"Rcpp/enviarCorreo/",
                data:{ idcli:idClientex, idcpp:foliocpp, destinatario:correox, copia:copiax, otro_correo:correox2 },
                cache: false,
                success: function(result)
                {
...
function showExcel() {
  ...
  $.ajax({
    ...
    url: base_urlx + "tw/php/crear_excel_complemento_pago.php",
    ...
    success: function (result) {
      ...
      if (result) {
        ...
        window.location.href = base_urlx + "tw/php/reporte/reporte_complementos_pago.xlsx";
...
function showPDFfactura(idfacturax){
      ...
      $.ajax({
          ...
          type: "POST",
          dataType: "json",
          url: base_urlx+"tw/php/pdf_acuse_cancelacion_cpp.php",
          data:{ 
          ...
          success: function(result)
          {
            ...
            window.open(base_urlx+"tw/php/facturas_canceladas_cpp/cancelacion_"+result+".pdf", "_blank");
            alert("EL complemento de pago "+result+" se cancelo con exito");
            table.ajax.reload();
            CierraPopup();
...
function cancelarFact(){
    ...
    if ( idcpp > 0 ) {
      ...
      x = confirm("¿Realmente deseas cancelar el COMPLEMENTO DE PAGO "+fol_cpp+"?");
      ...
      if (x){
        ...
        $.ajax({
              type: "POST",
              dataType: "json",
              url: base_urlx+"tw/php/cancelar_cpp.php",
              data:{ 
              ...
              success: function(result)
              {
                ...
                if ( result ) {
                  ...
                  showPDFfactura(idcpp);

                  //alert("EL CPP "+fol_cpp+" se cancelo con exito");
                  //table.ajax.reload();
                  //CierraPopup();
...
function generarZip(){
  ...
  if ( fol_cpp != "" ) {
    ...
    $.ajax({
            type: "POST",
            dataType: "json",
            url: base_urlx+"tw/php/zip_cpp.php",
            data:{folio:fol_cpp},
            cache: false,
            success: function(result)
            {
              ...
              if ( result ) {
                ...
                window.open(base_urlx+"tw/php/zip_facturas_cpp/"+fol_cpp+".zip" , "_blank");
