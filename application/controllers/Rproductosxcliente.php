...
class Rproductosxcliente extends CI_Controller

{
...

    {
        ...
        if ( $verificar_menu > 0 ) {
            ...
            $dmenu["nommenu"] = "rproductoxcliente";
            ...
            if($iduser > 0){
                ...
                $this->load->view('reportes/body_productoxcliente');

                $this->load->view('general/footer');

                $this->load->view('reportes/acciones_productoxcliente');
    ...
    public function lista_ventaxproducto(){
        ...
        $sql='SELECT a.cantidad,e.costo,CONCAT_WS("",f.serie,f.folio) AS fol_factura,g.descripcion AS unidad
            FROM partes_factura a, alta_productos b, alta_factura c, alta_marca d, partes_cotizacion e, folio_factura f,sat_catalogo_unidades g
            WHERE a.idparte=b.id
            AND a.idfactura=c.id
            AND b.idmarca=d.id
            AND a.idpartecot=e.id
            AND c.idfolio=f.id
            AND b.idunidad=g.id
            AND c.idcliente='.$data_post["idcli"].'
            AND c.estatus=1
            AND a.idparte='.$data_post["idproducto"].' ORDER BY f.folio DESC';
