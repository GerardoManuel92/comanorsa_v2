...
class AltaRq extends CI_Controller

{
...

    {
        ...
        function almacenTotal($idpartey){
            ...
            $class = $ci->db->query("
                ...
                IFNULL( (SELECT SUM(psr.asignado_almacen) FROM partes_solicitar_rq psr WHERE psr.estatus = 0 AND psr.idparte= a.id),0) AS totasignado
                FROM alta_productos a, sat_catalogo_unidades b 
                WHERE
                a.idunidad=b.id 
                AND a.id=".$idpartey."
...

    {
        ...
        if ($verificar_menu > 0) {
            ...
            if ($iduser > 0) {
                ...
                $sqlx="SELECT pao.id AS id, almacenasg AS almacenado, pao.idparte AS idparte, pao.idpartecot AS idpartecot
                        FROM partes_asignar_oc pao
                        WHERE pao.estatus = 0";
                ...
                if ($qdatos!=null) {
                    ...
                    foreach ($qdatos as $qrow) {
                        ...
                        $almacenDisponible = almacenTotal($qrow->idparte);

                        if($almacenDisponible > 0){
                            //Existe almacen disponible                            

                            $idRQ = "SELECT * FROM partes_solicitar_rq WHERE idpartecot =".$qrow->idpartecot;
                            $infoId = $this->General_Model->infoxQueryUnafila($idRQ); 
                            
                            $faltante = $infoId->cantidad - $infoId->asignado_almacen;

                            if($faltante > 0){
                                if($qrow->almacenado == 0){
                                    //No se ha asignado nada del almacen
    
                                    if($almacenDisponible >= $faltante){
                                        $totalAsignar = $infoId->asignado_almacen + $faltante; 
                                    }else{
                                        $totalAsignar = $infoId->asignado_almacen + $almacenDisponible; 
                                    }
                                   
                                    $dataRQ = array('asignado_almacen' => $totalAsignar);
                                    $tableRQ = "partes_solicitar_rq";
                                    $conditionRQ = array('id' => $infoId->id);
                                    $this->General_Model->updateERP($dataRQ, $tableRQ, $conditionRQ);
                                    
                                    $datos = array('almacenasg' => 1);
                                    $tabla = "partes_asignar_oc";
                                    $condicion = array('id' => $qrow->id);
                                    $this->General_Model->updateERP($datos, $tabla, $condicion);
    
                                }else{
                                    //Se asigno desde almacen
                        ...
                        /* idpartecor, cantidad, ASidparte, costo_proveedor, precio, iva, descuento */
                        /* id, idpartecot, cantidad, idparte, costo_proveedor, costo, precio, iva, descuento, estatus*/
                ...
                // CARGA DE ALMMACEN AUTOMATICAMENTE ----- END -----
                ...
                $this->load->view('requerimientos/alta_asignar_rq');

                $this->load->view('general/footer');

                $this->load->view('requerimientos/acciones_asignar_rq');
            } else {
    ...
    public function asignarPartidas()
    {
        ...
        $idRQ = "SELECT MAX(id) AS id FROM alta_rq";
        $infoId = $this->General_Model->infoxQueryUnafila($idRQ);        
        
        foreach ($dataArrayid as $arrayid) {

            $sqlInfo = "SELECT * FROM partes_solicitar_rq WHERE id=".$arrayid;
            $info_rq = $this->General_Model->infoxQueryUnafila($sqlInfo);

            if($info_rq->cantidad_rq == 0){
                // En caso de no aceptar requerimientos igual a 0
            }

            $insertToRq = array('fecha' => date("Y-m-d"), 'hora' => date("H:i:s"), 'idcot' =>$info_rq->idcot, 'idpartecot' => $info_rq->idpartecot, 'cantidad' => $info_rq->cantidad, 'cantidad_rq' =>$info_rq->cantidad_rq, 'idparte'=> $info_rq->idparte, 'idrq' => $infoId->id);
            $inTableRq = "partes_rq";
            $this->General_Model->altaERP($insertToRq,$inTableRq);

            $idPRQ = "SELECT pr.id AS id, pc.costo_proveedor AS costo_proveedor, pc.costo AS costo,
                        pr.idparte AS idparte, pc.iva AS iva, pc.descuento AS descuento, pr.cantidad_rq AS cantidad
                        FROM partes_rq pr, partes_cotizacion pc
                        WHERE pr.idpartecot = pc.id
                        ORDER BY id DESC LIMIT 1";
            $infoIdPRQ = $this->General_Model->infoxQueryUnafila($idPRQ);  

            $insertToCA = array('fecha' => date("Y-m-d"), 'iduser' => $data_post["iduser"], 'idparterq' => $infoIdPRQ->id, 'idproveedor'=>0, 'idoc'=>0, 'idparte'=>$infoIdPRQ->idparte, 'costo_proveedor'=>$infoIdPRQ->costo_proveedor, 'costo_final'=>$infoIdPRQ->costo, 'precio'=>$infoIdPRQ->costo, 'precio_final'=>calcularPrecio($infoIdPRQ->costo,$infoIdPRQ->iva,$infoIdPRQ->descuento),'iva'=>$infoIdPRQ->iva, 'descuento'=>$infoIdPRQ->descuento, 'cantidad'=>$infoIdPRQ->cantidad, 'solicitar'=>$infoIdPRQ->cantidad, 'estatus'=>0 );
            $inTableCA = "partes_costos_asignar";
            $this->General_Model->altaERP($insertToCA,$inTableCA);

            $sqlCantidad = "SELECT psr.cantidad AS cantidad, (IFNULL((SELECT SUM(pr.cantidad_rq) FROM partes_rq pr WHERE pr.idpartecot = psr.idpartecot),0)) AS cantidad_rq, cantidad_rq AS cantidad_solicitada FROM partes_solicitar_rq psr WHERE psr.id=".$arrayid;
            $info_data = $this->General_Model->infoxQueryUnafila($sqlCantidad);
            
            if(($info_data->cantidad_rq) >= ($info_data->cantidad)){
                $udatos=array('estatus'=> 1);
                $itabla="partes_solicitar_rq";
                $ucondicion=array('id'=> $arrayid);
    ...
    public function updateCelda()
    {
        ...
        switch ($data_post["columna"]) {
            case 8:
                ...
                /*$solicitado = $data_post["texto"] - $data_post["almacen"];
                if ($solicitado > 0) {
                    $diferencia = $data_post["almacen"] - $solicitado;
                   if ($diferencia >= 0 and $data_post["texto"]) {
                        ...
                        foreach ($partes as $row) {
                            $pdatos = array('fecha' => date("Y-m-d"), 'hora' => date("H:i:s"), 'idasoc' => $data_post["idpcot"], 'idcot' => $row->idcot, 'idpartecot' => $row->idpartecot, 'idoc' => 0, 'cantidad' => $data_post["texto"], 'cantidad_oc' => $row->cantidad_oc, 'idparte' => $row->idparte, 'idproveedor' => $data_post["idproveedor"], 'costo_proveedor' => $row->costo_proveedor, 'costo' => $row->costo, 'iva' => $row->iva, 'descuento' => $row->descuento);
                            $pdatos2 = array('fecha' => date("Y-m-d"), 'hora' => date("H:i:s"), 'idcot' => $row->idcot, 'idpartecot' => $row->idpartecot, 'idoc' => 0, 'cantidad' => $diferencia, 'cantidad_oc' => $row->cantidad_oc, 'idparte' => $row->idparte, 'idproveedor' => 1, 'costo_proveedor' => $row->costo_proveedor, 'costo' => $row->costo, 'iva' => $row->iva, 'descuento' => $row->descuento);
                            $table3 = "partes_asignar_oc";
                            $table4 = "alta_producto_proveedor";
                            $this->General_Model->altaERP($pdatos2, $table3);
                            $this->General_Model->altaERP($pdatos, $table4);                            
                        }
                    } else if($diferencia < 0) {   */   
                        // $data_post["idpcot"] -> ID de la parte_asignar_oc 


                        //// verificar si ya existe el idparte en el temporal

                        /*$itabla="temporal_partes_asignar_oc";
                        $vcondicion=array('idparte'=> $data_post["idpcot"]);
                        $repetida=0;

                        $repetida=$this->General_Model->verificarRepeat($itabla,$vcondicion);

                        if($repetida>0) {
                            
                            ////idparte ya existe en la tabla temporal

                            

                        }else{

                            ///idparte no esta en la tabla
                            /*$idatos=array('cantidad' => $data_post["texto"], 'idparte'=> $data_post["idpcot"], 'iduser'=> $data_post["idusuario"] );

                            $this->General_Model->altaERP($idatos,$itabla);*/
            ...
            case 7:
                ...
                if (quitarPesos($data_post["texto"]) > 0) {
                    ...
                    /*$upData= array('costo' => quitarPesos($data_post["texto"]));    
                    $upTable = "partes_asignar_oc";        
                    $upCondition = array('id' => $data_post["idpcot"] );                    
                    ...
                    if($repetida>0){
                        ...
                        $udatos=array('costo' => quitarPesos($data_post["texto"]));
                        $ucondicion=array('id'=> $data_post["idpcot"]);
                        $this->General_Model->updateERP($udatos,$itabla,$ucondicion);
...

    {
        ...
        if ($data_get["cot"] > 0) {
            ...
            $table = "(SELECT psr.id AS id, ap.nparte AS clave, CONCAT_WS('||',ap.nparte,ap.descripcion) AS descripcion,
                        ...
                        AND psr.idproveedor = 0 AND psr.estatus = 0
                        AND psr.idcot =" . $data_get['cot'] . ")temp ";
