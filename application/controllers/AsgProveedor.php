...
class AsgProveedor extends CI_Controller

{
...

    {
        ...
        if ($verificar_menu > 0) {
            ...
            if ($iduser > 0) {
                ...
                $this->load->view('compras/alta_asignar_proveedor');

                $this->load->view('general/footer');

                $this->load->view('compras/acciones_asignar_proveedor');
            } else {
    ...
    public function asignarParteOC(){
        ...
        for ($i = 0; $i < sizeof($arrayid); $i++){                        
            ...
            $query = "SELECT pca.id AS id, pr.idpartecot AS idpartecot, pca.idparte AS idparte, 
                        pca.solicitar AS cantidad, pca.costo_final AS costo, pca.iva AS iva, 
            ...
            $inData2 = array('idoc'=>'0', 'idpartecosto'=>$resultQuery->id, 'idpartecot'=>$resultQuery->idpartecot, 'idparte'=>$resultQuery->idparte, 'cantidad'=>$resultQuery->cantidad, 'costo'=>$resultQuery->costo, 'iva'=>$resultQuery->iva, 'descuento'=>$resultQuery->descuento, 'precio'=>$resultQuery->precio, 'idproveedor'=>$idproveedor);
            $inTable2 = "partes_oc";
            $this->General_Model->altaERP($inData2,$inTable2);
            
            // UPDATE ESTATUS

            $queryCantidades = "SELECT IFNULL((SELECT SUM(po.cantidad) FROM partes_oc po WHERE po.estatus = 0 AND po.idpartecosto =pca.id),0) AS asignado,
                                pca.cantidad AS cantidad
                                FROM partes_costos_asignar pca
                                WHERE pca.id =".$arrayid[$i];
            $infoCantidades = $this->General_Model->infoxQueryUnafila($queryCantidades);

            $udatos = "";

            if(($infoCantidades->cantidad - $infoCantidades->asignado)==0){
                $udatos=array('idproveedor' => $idproveedor, 'estatus' => 2);
            }else{
                $udatos=array('idproveedor' => $idproveedor);
            }                    
    ...
    public function updateCelda()
    {
        ...
        switch ($data_post["columna"]) {
            ...
            case 9:
                /// IVA
                $udatos=array('iva' => $data_post["texto"]);
                $itabla="partes_costos_asignar";
                $ucondicion=array('id'=> $data_post["idprq"]);
                $this->General_Model->updateERP($udatos,$itabla,$ucondicion);                  
            break;
            case 10:
                ...
                if (quitarPesos($data_post["texto"]) > 0) {                
                    $udatos=array('costo_final' => quitarPesos($data_post["texto"]));
                    $itabla="partes_costos_asignar";
                    $ucondicion=array('id'=> $data_post["idprq"]);
                    $this->General_Model->updateERP($udatos,$itabla,$ucondicion);                
                } 
...

    {
        ...
        if ($data_get["rq"] > 0) {
            ...
            $table = "(SELECT pca.id AS id, pca.idparterq AS parterq, ap.nparte AS nparte, CONCAT_WS('||',ap.nparte,ap.descripcion) AS descrip,
                        pca.solicitar AS solicitar, pca.costo_proveedor AS costo_venta, pca.precio AS precio_venta, pca.descuento AS descuento, 
                        pca.iva AS iva, pca.costo_final AS costo_final, pca.precio_final AS precio_final,
                        CONCAT_WS('/',(IFNULL((SELECT SUM(po.cantidad) FROM partes_oc po WHERE /*po.estatus = 0 AND*/ po.idpartecosto = pca.id),0)),pca.cantidad) AS cantidad
