...
class Rcpp extends CI_Controller
{
  ...
  public function __construct()
  {
    ...
    function Mptotal($idcppx)
    {

      $ci = &get_instance();

      $class = $ci->db->query('SELECT a.idfactura, CONCAT_WS("",c.serie,c.folio) AS facturax
                                                          FROM alta_pagos_ppd a, alta_factura b, folio_factura c
      ...
      foreach ($class as $row) {
        ...
        $resultx .= '<li><a href="' . base_url() . 'tw/php/facturas/' . $row["facturax"] . '.pdf" target="_blank"> ' . $row["facturax"] . '</a></li>';
      }
  ...
  public function enviarCorreo()
  {
    ...
    $this->email->from(CORREOVENTAS, 'Comanorsa complemento de pago Cpp' . $idcpp . ' - ' . $datos_cliente->nombre);
    //$this->email->to("thinkweb.mx@gmail.com");

    $this->email->to(trim($destinatariox));
    ...
    $this->email->cc("thinkweb.mx@gmail.com");

    $this->email->subject('Comanorsa complemento de pago Cpp' . $idcpp . ' - ' . $datos_cliente->nombre);
    ////ADJUNTAR ARCHIVOS
    //$adjunto= base_url()."admin/files/admin/js/upload/files/".$this->input->post('nombre'); 
    //$this->email->attach($adjunto);
    $this->email->attach('tw/php/facturas_ppd/CP' . $idcpp . '.pdf');
    $this->email->attach('tw/php/factura_comprobante_pago/CP' . $idcpp . '.xml');
  ...
  public function loadPartidas()
  {
    ...
    if ($data_get["buscar"] == "" && $data_get["estatusx"] == 0) {

      $table = "( SELECT CONCAT_WS('/',a.id,a.estatus,IFNULL(f.serie,''),IFNULL(f.folio,'') ) AS acciones, a.id, a.estatus,a.fecha AS ftimbrado,b.nombre AS usuario,e.nombre AS cliente, a.total, g.clave AS monedax, CONCAT_WS('',f.serie,f.folio) AS fol_cpp, e.id AS idcliente, h.folio AS folio_cpp, ( SELECT x.id FROM alta_datos_ppd x WHERE x.idcpd=a.id ) AS ippdx
                      FROM alta_ppd a
                      LEFT JOIN folio_ppd f ON a.idfolio=f.id,
                      alta_usuarios b, alta_clientes e, alta_moneda g, folio_ppd h
                      WHERE
                      a.iduser=b.id
                      AND a.idcliente=e.id
                      AND a.idmoneda=g.id
                      AND a.idfolio=h.id
                      ORDER BY a.idfolio DESC 
                      LIMIT 0,100)temp";
    } else if ($data_get["buscar"] != "" && $data_get["estatusx"] == 0) {
      ...
      $table = "(SELECT CONCAT_WS('/',a.id,a.estatus,IFNULL(f.serie,''),IFNULL(f.folio,'')) AS acciones, a.id, a.estatus,a.fecha AS ftimbrado,b.nombre AS usuario,e.nombre AS cliente, a.total, g.clave AS monedax, CONCAT_WS('',f.serie,f.folio) AS fol_cpp, e.id AS idcliente, h.folio AS folio_cpp, ( SELECT x.id FROM alta_datos_ppd x WHERE x.idcpd=a.id ) AS ippdx
                      ...
                      AND a.idmoneda=g.id
                      AND a.idfolio=h.id                      
                      AND CONCAT_WS(' ',e.nombre,e.comercial,f.folio,a.uuid,a.total) LIKE '%" . $data_get["buscar"] . "%' 
                      ORDER BY a.idfolio DESC)temp";
    } else if ($data_get["buscar"] != "" && $data_get["estatusx"] > 0) {
    ...
    $columns = array(
              ...
              return '<div class="btn-group">
                                    ...
                                    <span class="caret"></span>
                                      ...
                                      <ul class="dropdown-menu">
                                        ...
                                        <li><a href="javascript:verEstatus(' . $separar[0] . ',2)" style="color:blue; font-weight:bold;"><i class="fa fa-eye"></i> Ver estatus</a></li>
              ...
              return '<div class="btn-group">
                                    ...
                                    <span class="caret"></span>
                                      ...
                                      <ul class="dropdown-menu">
                                        ...
                                        <li><a href="' . base_url() . 'tw/php/facturas_canceladas_cpp/cancelacion_' . $separar[2] . '' . $separar[3] . '.pdf" target="_blank" style="color:red; font-weight:bold;"><i class="fa fa-eye"></i> Ver acuse</a></li>
      ...
      array(
        ...
        'formatter' => function ($d, $row) {
          ...
          $separarx = explode("CP", $d);

          return '<a href="' . base_url() . 'tw/php/facturas_ppd/' . $d . '.pdf" target="_blank">' . $separarx[1] . '</a>';
        }
      ),
  ...
  public function loadPartidasxEstatus()
  {

    $iduser = $this->session->userdata(IDUSERCOM);
    $data_get = $this->input->get();

    if ($data_get["estatusx"] == 0) {
