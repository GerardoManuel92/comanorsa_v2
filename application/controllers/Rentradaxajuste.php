...
class Rentradaxajuste extends CI_Controller

{
...

    {
        ...
        if ( $verificar_menu > 0 ) {
          ...
          if($iduser > 0){
              ...
              $nom_menu["nommenu"] = "rentradaxajuste";
              ...
              $this->load->view('entradas/reporte_entradasxajuste');

              $this->load->view('general/footer');

              $this->load->view('entradas/accciones_reporte_entradasxajuste');
    ...
    public function cancelarAjuste(){
        ...
        $query='UPDATE alta_ajuste_entrada SET estatus = 1 WHERE id ='.$data_post["id"];
    ...
    public function showPartidas(){
        ...
        FROM partes_ajuste_entrada a, alta_productos b, alta_ajuste_entrada c
        WHERE
        a.idparte=b.id
        AND c.id = a.idajuste
    ...
    ///////////****************TABLA PARTIDAS BITACORA
...

        {
            ...
            if($data_get["idpro"] == 0 && $data_get["statusx"] == 2) {
                
                $table = "(SELECT a.id,a.fecha,a.total,b.nombre AS almacenista, c.nombre AS proveedor, CONCAT_WS('/',a.id,a.estatus) AS acciones, a.estatus, a.observaciones as motivo, a.documento   
                        FROM alta_ajuste_entrada a, alta_usuarios b, alta_proveedores c, partes_ajuste_entrada d
                        WHERE
                        a.idusuario=b.id AND d.idajuste = a.id
                        AND a.idproveedor=c.id                        
                        GROUP BY a.id)temp";

            }else if($data_get["idpro"] == 0 && $data_get["statusx"] != 2) {
