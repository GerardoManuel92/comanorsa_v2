...
class Ajusteinventario extends CI_Controller

{
    ...
    * @see https://codeigniter.com/user_guide/general/urls.html
...

    {
        ...
        if ( $verificar_menu > 0 ) {
            ...
            if($iduser > 0){
                ...
                $this->load->view('entradas/body_ajustexentrada');

                $this->load->view('general/footer');

                $this->load->view('entradas/acciones_ajustexentrada');
    ...
    public function retirarParte(){
        ...
        $tabla="temporal_partes_ajuste_entrada";
    ...
    public function ingresarPartidas(){
        ...
        $tabla1 = "temporal_partes_ajuste_entrada";
        ...
        if ( $repeat == 0 ) {
            ...
            $table = "temporal_partes_ajuste_entrada";
    ...
    public function finalizarAjuste(){
        ...
        $rtabla="temporal_partes_ajuste_entrada";
        ...
        if ($repeat>0) {
            ...
            $table = "alta_ajuste_entrada";

            $last_id=$this->General_Model->altaERP($data,$table);

            if ( $last_id > 0 ) {

                $sqlx = "SELECT id,fecha,idusuario,idparte,cantidad,costo,iva,tot_iva,tot_total,estatus FROM `temporal_partes_ajuste_entrada` WHERE idusuario=".$data_post['iduser']." AND estatus=0";
                ...
                foreach ($partes as $row) {
                    ...
                    $table3 = "partes_ajuste_entrada";
                    $parteEntrada = $this->General_Model->altaERP($data3,$table3);

                    // ENTRADA PEPS
                    if ( $parteEntrada > 0 ){
                        $dataPEPS = array('fecha' => date("Y-m-d"), 'hora'=> date("H:i:s"), 'idmovimiento' => $parteEntrada, 'idparte' =>  $row->idparte, 'movimiento' => 0, 'cantidad' => $row->cantidad, 'estatus' => 0, 'cantidad_consumida' =>0, 'costo'=>$row->costo);                  
                        $tablePEPS = "entrada_peps";
                        $this->General_Model->altaERP($dataPEPS,$tablePEPS);
                    }                                        
                    // ENTRADA PEPS
                ...
                $tabla4 = "temporal_partes_ajuste_entrada";
    ...
    public function loadPartidas()

        {
            ...
            $pregunta = array();
                ...
                FROM temporal_partes_ajuste_entrada a, alta_productos b, sat_catalogo_unidades c, alta_marca d
